import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import shebeiyuyue from '@/views/modules/shebeiyuyue/list'
    import jiaoxueshijian from '@/views/modules/jiaoxueshijian/list'
    import kechengbiao from '@/views/modules/kechengbiao/list'
    import yudingshiyanshi from '@/views/modules/yudingshiyanshi/list'
    import shiyanshixinxi from '@/views/modules/shiyanshixinxi/list'
    import yuyuekecheng from '@/views/modules/yuyuekecheng/list'
    import shebeixinxi from '@/views/modules/shebeixinxi/list'
    import xueshengqiandao from '@/views/modules/xueshengqiandao/list'
    import config from '@/views/modules/config/list'
    import yonghuxinxi from '@/views/modules/yonghuxinxi/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/shebeiyuyue',
        name: '设备预约',
        component: shebeiyuyue
      }
      ,{
	path: '/jiaoxueshijian',
        name: '教学实践',
        component: jiaoxueshijian
      }
      ,{
	path: '/kechengbiao',
        name: '课程表',
        component: kechengbiao
      }
      ,{
	path: '/yudingshiyanshi',
        name: '预定实验室',
        component: yudingshiyanshi
      }
      ,{
	path: '/shiyanshixinxi',
        name: '实验室信息',
        component: shiyanshixinxi
      }
      ,{
	path: '/yuyuekecheng',
        name: '预约课程',
        component: yuyuekecheng
      }
      ,{
	path: '/shebeixinxi',
        name: '设备信息',
        component: shebeixinxi
      }
      ,{
	path: '/xueshengqiandao',
        name: '学生签到',
        component: xueshengqiandao
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
      ,{
	path: '/yonghuxinxi',
        name: '用户信息',
        component: yonghuxinxi
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
